...
if (isset($_POST['idCom'])) {
    $idCom = htmlspecialchars(trim($_POST['codeLCom']));
    $data = array('idCom' => $idCom);
    $result = CallAPI('DELETE', "deleteCom.php", $data);
}
...
if (isset($_POST['comment'])) {
    $com = htmlspecialchars(trim($_POST['commentai']));

    if (isset($com) and !empty($com)) {
        $ins = $dbb->prepare("SELECT * FROM commentaire WHERE comment= ?");
        $ins->execute([$com]);
        $verif = $ins->rowCount();

        if ($verif == 0) {
            $req = $dbb->prepare("INSERT INTO commentaire(comment) VALUES(?)");
            $req->execute([$com]);
            $erreur = "Merci pour l'enregistrement";
        }
    } else if ($verif == 1) {
        $erreur = "Vous avez dejà un commentaire Merci";
    }
} else {
...
$inscrit = json_decode(file_get_contents("http://localhost:8888/api/controllers/readAllCom.php"));
// print_r($inscrit);
...
<body>
    ...
    <div class="breadcrumb-option" style="
        background: url(image/commentaire.jpeg) no-repeat;
            ...
            <div class="row">
                ...
                <div class="col-lg-6 offset-lg-1 col-md-6 col-sm-7">
                    <div class="contact__form">
                        ...
                        <form action="#" method="POST">
                            <input type="text" name="commentai" id="" placeholder="Commentaire">
                            <button name="comment" type="submit">Envoyer Commentaire</button>
                        </form>
                    </div>
                </div>
                <div class="col-lg-12 offset-lg-1 col-md-6 col-sm-7">
                    <div class="contact__form" style="margin-bottom:400px">
                        ...
                        <!-- Soit on mettra des -->
                        <div>
                            <?php foreach ($inscrit[0] as $inscrits) : ?> <div class="info">
                                <h3>
                                    <?= $inscrits->idCom ?></h3>
                                <p>
                                    <?= $inscrits->comment ?>
                                ...
                                <div style="display:flex">
                                    <a href="#" class="btn btn-primary" style="height:50px">Ajouter</a>
                                    <?php
                                        echo ' <form action="./commentaire.php" method="POST"><input type="hidden"
                                name="codeLCom" value=' . $inscrits->idCom . ' >
                                <input type="submit" class="btn btn-primary Hu" style="width:118px; height:50px" name="idCom"  value="Supprimer">
                                </form> '
                                        ?>
                                </div>
                            </div>
